...
Haddad et al. (2021) 提出了多区域 MFD 模型与去中心化自适应边界控制，处理了时延与不确定性，但其控制目标未与网络规划衔接。Weng et al. (2025) 建立了双层架构（SOFA + MFD 控制），实现了静态-动态桥接，但仅限于两区域且未考虑路径规划层对拥堵的主动规避。本文借鉴上述研究的区域级 MFD 控制和静态-动态桥接思想，但不引入飞行时间延迟、自适应律或显式起降排队状态，而是在无时延理想环境下构造最简多区域理论模型。具体而言，本文引入基于 MFD 速度衰减机制的空域拥堵感知路径规划层，并建立静态-动态之间的解析映射，缓解多区域 UAM 网络管理中层间协同的难题。与两篇源论文相比，本文重新定义了标量周界控制变量：$u_I$ 调节区域 $I$ 的总流出率，静态路由比例 $\beta$ 负责流向分配。该分工避免了逐边界控制的归一化矛盾，并使多区域稳定性分析可在无时延的基础空域子系统上展开。

本文中的拥堵感知并不表示两个彼此独立且已被证明必然带来性能增益的机制叠加，而是指同一 MFD 速度衰减信息在两个规划层面的复用：静态 SO 层通过密度相关空域飞行时间修正 OD 服务流量分配，路径规划层通过速度-密度旅行时间修正候选路径比例。该机制的收益依赖网络拓扑、需求水平和 MFD 参数标定，因此本文仅给出可检验的算法闭合条件，并在数值方案中通过消融实验评估其效果。与 Weng et al. 两区域自由流模型中的全局分区收敛结论不同，本文的多区域稳定性结论是针对固定路由比例、固定外部输入的无时延基础空域子系统的局部判据。
...
为了在规划阶段量化区域拥堵成本，我们采用参考文献中的广义指数型速度衰减结构。设 $n_I$ 为区域 $I$ 内的航空器累积量，该区域内的平均飞行速度 $v_I(n_I)$ 建模为：
\begin{equation} \label{eq:speed_density}
    v_I(n_I)
    =
    v_{f,I}
    \exp\left[
        -\frac{1}{\xi_I}
        \left(\frac{n_I}{n_{I,cr}}\right)^{\xi_I}
    \right],
    \quad v_{f,I}>0,\ n_{I,cr}>0,\ \xi_I>0 .
\end{equation}
其中 $v_{f,I}$ 为区域 $I$ 的自由流速度，$n_{I,cr}$ 为 MFD 最大产出对应的临界累积量，$\xi_I$ 为形状参数。为避免与路径比例 $\alpha_m^{(p)}$ 混淆，本文将参考文献公式中的形状参数 $\alpha$ 记为 $\xi_I$。

\begin{property}[速度-密度模型性质]\label{prop:speed_density}
函数 $v_I(n_I)$ 满足：
\begin{enumerate}
    \item $v_I(0) = v_{f,I}$；
    \item $v_I(n_I)$ 在 $n_I>0$ 上关于 $n_I$ 严格递减；
    \item $\lim_{n_I \to \infty} v_I(n_I) = 0$。
\end{enumerate}
\end{property}

\subsection{广义指数型 MFD 产出函数}\label{subsec:mfd_derivation}

\begin{definition}[MFD 产出函数]\label{def:mfd}
区域 $I$ 的宏观基本图（MFD）产出函数定义为航空器积累量与区域总流出率之间的关系：
\begin{equation}\label{eq:mfd_from_speed}
    G_I(n_I)
    \equiv
    \frac{v_{f,I}n_I}{A_I}
    \exp\left[
    ...
    \right]
    =
    \frac{n_I \cdot v_I(n_I)}{A_I}
\end{equation}
其中 $A_I>0$ 为区域 $I$ 的等效平均穿越尺度，不同于后文线性化稳定性分析中的矩阵 $A$。若 $v_{f,I}$ 的量纲为长度/时间、$n_I$ 的量纲为航空器、$A_I$ 的量纲为长度，则 $G_I$ 的量纲为航空器/时间，与 MFD 流出率一致。
\end{definition}

\begin{assumption}[等效穿越尺度一致性]\label{asm:representative_length}
路径层的密度固定点和动态层的 MFD 产出函数使用同一组区域速度函数 $v_I(\cdot)$。本文主模型将 $A_I$ 视为区域级外部标定参数，并在 SO--路径迭代、稳态目标构造和控制稳定性分析中保持固定；因此 $G_I(n_I)$、$G_I^{\max}$ 和 $n_I^{cr,*}$ 均由该固定标定值确定。为量化路径层 Little 定律密度与动态 MFD 产出之间的严格一致性偏差，对每个区域 $I$，若给定路径比例 $\alpha_m^{(p)}$ 和服务流量 $f_m$ 时区域通过流量为
\begin{equation}
    F_I=\sum_{m\in\mathcal M}\sum_{p=1}^{P_m}
    \alpha_m^{(p)}f_m\mathbf 1\{L_{I,m}^{(p)}>0\},
\end{equation}
则可定义诊断用的流量加权等效穿越尺度
\begin{equation}\label{eq:representative_length}
    A_I^{fw} =
    \frac{\sum_{m\in\mathcal M}\sum_{p=1}^{P_m}\alpha_m^{(p)}f_m L_{I,m}^{(p)}}{F_I},
    \quad F_I>0 .
\end{equation}
当 $F_I=0$ 时，$A_I^{fw}$ 不参与诊断，因为该区域不承担正吞吐流。若固定标定值满足 $A_I=A_I^{fw}$，则路径层 Little 定律密度与 MFD 产出在流量加权意义下严格一致；若二者存在偏差，则后续稳态桥接结论应理解为在固定标定 MFD 下的宏观近似闭合，而非逐路径旅行时间的严格恒等式。该偏差及其对容量、目标状态和稳定性判据的影响需在数值实验中报告。
\end{assumption}

\begin{theorem}[MFD 产出函数的 Unimodal 性质]\label{thm:mfd_unimodal}
由式 (\ref{eq:mfd_from_speed}) 定义的广义指数型 MFD 产出函数满足以下性质：
\begin{enumerate}
    \item $G_I(0) = 0$；
    \item $G_I(n_I)$ 在 $(0, +\infty)$ 上存在唯一的临界点 $n_I^{cr,*}=n_{I,cr}$，使得 $G_I$ 在 $(0, n_I^{cr,*})$ 上严格递增，在 $(n_I^{cr,*}, +\infty)$ 上严格递减，即 $G_I$ 为 unimodal 函数；
    \item $G_I$ 在 $n_I = n_I^{cr,*}$ 处取得全局最大值
    \begin{equation}
        G_I^{\max}
        =
        \frac{v_{f,I}n_{I,cr}}{A_I}
        \exp\left(-\frac{1}{\xi_I}\right).
    \end{equation}
\end{enumerate}
\end{theorem}
...
\begin{equation}
    ...
    \right]
    \left[
        1-\left(\frac{n_I}{n_{I,cr}}\right)^{\xi_I}
    \right].
\end{equation}
由于 $v_{f,I}/A_I>0$ 且指数项恒为正，导数符号只由 $1-(n_I/n_{I,cr})^{\xi_I}$ 决定。因此当 $0<n_I<n_{I,cr}$ 时 $dG_I/dn_I>0$，当 $n_I=n_{I,cr}$ 时 $dG_I/dn_I=0$，当 $n_I>n_{I,cr}$ 时 $dG_I/dn_I<0$。故 $G_I$ 为 unimodal，唯一最大点为 $n_I^{cr,*}=n_{I,cr}$。将 $n_I=n_{I,cr}$ 代入式 (\ref{eq:mfd_from_speed})，得到 $G_I^{\max}=\frac{v_{f,I}n_{I,cr}}{A_I}\exp(-1/\xi_I)$。
\end{proof}

\begin{definition}[MFD 自由流分支与拥堵分支]
\begin{itemize}
    \item \textbf{自由流分支}：$G_{I,s}(n_I) = G_I(n_I)$，$n_I \in [0, n_I^{cr,*}]$，严格递增，可逆；
    \item \textbf{拥堵分支}：$G_{I,u}(n_I) = G_I(n_I)$，$n_I \in (n_I^{cr,*}, +\infty)$，严格递减，可逆。
\end{itemize}
\end{definition}

\begin{remark}
为保持后续目标构造、容量检查和稳定性表述的统一，本文继续使用 $n_I^{cr,*}$ 表示 MFD 最大产出点。在广义指数型 MFD 下，该点有显式表达 $n_I^{cr,*}=n_{I,cr}$，不再需要求解隐式临界方程。
\end{remark}
...
\begin{equation}\label{eq:transfer_flux}
    ...
    \end{cases}
\end{equation}
此处 $G_I(\cdot)$ 为第 \ref{subsec:mfd_derivation} 节推导的 MFD 产出函数；$\beta_{I \to K}^D$ 为静态分流比例（性质 \ref{prop:beta_normalization}）；$u_I(t) \in [0, 1]$ 为区域 $I$ 的\textbf{标量周界控制变量}，统一调节该区域的总流出率。在性质 \ref{prop:beta_normalization} 的归一化条件下，每个区域的总流出满足
...
\begin{enumerate}
    ...
    \item \textbf{归一化自洽}：总流出 $\sum_{D,K} \mathcal{T}_{I \to K}^D = G_I(n_I) \cdot u_I$，当 $u_I = 1$ 时恰等于 MFD 产出，无归一化矛盾；
...
\end{equation}
由目标组分的定义 (\ref{eq:target_component})，对 $H\in\mathcal R_+$ 有 $\bar\pi_H^D = \frac{\sum_{K'} T_{H \to K'}^D}{\Lambda_H}$。代入并利用 $\beta_{H \to I}^D = T_{H \to I}^D / \sum_{K'} T_{H \to K'}^D$（分母为零时该目的地组分不贡献流量）：
\begin{equation}
\begin{aligned}
...
\end{equation}
其中 $G_I^{\max} = G_I(n_I^{cr,*})$ 为定理 \ref{thm:mfd_unimodal} 中的 MFD 最大值，零吞吐区域按定义 \ref{def:target_component} 处理，且跨区域转移矩阵 $P$ 满足 $\rho(P)<1$，则稳态方程组 (\ref{eq:steady_state_eq})--(\ref{eq:steady_transfer}) 存在至少一组解 $(\bar{n}_I, \bar{n}_I^D, \bar{u}_I)$，其中 $I\in\mathcal R_+$ 时 $\bar{u}_I \in (0,1)$，$I\notin\mathcal R_+$ 时 $\bar u_I=0$。
\end{theorem}
...
\begin{enumerate}
    ...
    \item 在均衡点邻域内控制不饱和且 $G_I(n_I)\ge\epsilon_I$，该邻域由注 \ref{rem:unsaturated_neighborhood} 的控制裕度保证；
    \item 初始扰动位于静态可达支持集 $\mathcal S$ 内，且线性化状态向量和矩阵 $A$ 的索引均限制在 $\mathcal S$ 上，满足假设 \ref{asm:support_consistency}；
...
\textbf{输入}：静态规划结果 $\{f_m^*, S_m^{(p)}, \alpha_m^{(p),*}\}$，MFD 参数 $\{G_I^{\max}, n_I^{cr,*}\}$，控制裕度 $\{\bar u_I\}_{I\in\mathcal R_+}$
...
\begin{itemize}
    ...
    \item \textbf{类型 II：全网守恒不一致}——全网外部需求 $\sum_I \bar{q}_{g,I}$ 不等于全网内部完成量 $\sum_{I,D} T_{I \to I}^D$。建议：检查路径规划层的候选路径集和OD需求分配，确保全网流量守恒。
...
\subsection{仿真网络设计}
构建包含 $N_R = 3$ 个区域的 UAM 网络拓扑，每个区域包含 2--3 个起降场。设定 3--5 组 OD 对，每组具有弹性需求，并为每个 OD 对生成 2--4 条不含区域环路的候选路径。仿真表格需至少列出：OD 需求上界与需求反函数参数、候选路径的区域序列和 $L_{I,m}^{(p)}$、起降场静态操作时间函数参数、MFD 参数和控制参数。MFD 参数采用可复现实验标定流程：先给定自由流速度 $v_{f,I}$、等效平均穿越尺度 $A_I$、临界累积量 $n_{I,cr}$ 和形状参数 $\xi_I$，再由定理 \ref{thm:mfd_unimodal} 取 $n_I^{cr,*}=n_{I,cr}$ 并计算 $G_I^{\max}=\frac{v_{f,I}n_{I,cr}}{A_I}\exp(-1/\xi_I)$。若引用 Weng et al. (2025) 或其他数据源的参数范围，应在表格中列出对应单位、换算关系和本文采用值；若缺少直接可迁移标定，则使用归一化参数并报告敏感性分析。
...
\begin{itemize}
    ...
    \item \textbf{等效穿越尺度一致性}：在 SO--路径算法终止后，用式 (\ref{eq:representative_length}) 计算 $A_I^{fw}$，报告相对偏差 $|A_I^{fw}-A_I|/A_I$，并通过扰动 $A_I$ 或替换为 $A_I^{fw}$ 评估其对 $G_I^{\max}$、$\bar n_I$、$\bar n_I^D$ 和谱横坐标 $\alpha_A$ 的影响；
...
\begin{itemize}
    ...
    \item 网络总吞吐量；
    \item 固定等效穿越尺度 $A_I$ 与流量加权尺度 $A_I^{fw}$ 的偏差，以及该偏差诱导的 $G_I^{\max}$、目标状态和 $\alpha_A$ 变化；
    \item 控制输入的平滑性与饱和频率；
...
\begin{enumerate}
    ...
    \item 路径层 Little 定律密度与动态 MFD 产出之间依赖等效穿越尺度一致性假设 \ref{asm:representative_length}；本文主模型固定标定 $A_I$，因此当 $A_I$ 与流量加权诊断尺度 $A_I^{fw}$ 存在偏差时，桥接结论应理解为宏观近似闭合。
